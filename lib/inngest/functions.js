        ...
        await step.run(`check-budget-${budget.id}`, async () => {
            ...
            const expenses = await db.transaction.aggregate({
                where: {
                    ...
                    accountId : defautAccount.id, //consider only default account
                    type: "EXPENSE",
                    date: {
                        gte: startOfMonth,
                        lte: endOfMonth
            ...
            if(percentageUsed >= 80 && (!budget.lastAlertSent || 
               ...
               //send the email to the user
                await sendEmail({
                    ...
                    subject: `Budget Alert for ${defautAccount.name}`,
                    react: EmailTemplate({
                        ...
                        data: {
                            ...
                            accountName: defautAccount.name,
                        },
                    })
                })
...
},
{
    ...
    async ({ step }) => {
        ...
        // step 2- create event for each recurring transaction in batches
        if (recurringTransactions.length > 0) {
            ...
            // step 3- Send events directly using inngest.send()
